...
for (ann in annotations) {
    def childCount = ann.nChildObjects()
    ...
    if (col >= 0 && col < cols && row >= 0 && row < rows) {
        fp.setf(col, row, childCount)
        countedCells += childCount
    }
}

// 6. Convert to an ImagePlus object for saving
ImagePlus imp = new ImagePlus("Cell Counts Per ${tileSizeMicrons}um Tile", fp)
...
mkdirs(outDir)
String outPath = buildFilePath(outDir, "tile_cell_counts_${tileSizeMicrons}um.tif")

// Save as a 32-bit TIFF
FileSaver fs = new FileSaver(imp)
fs.saveAsTiff(args?[0] != null ? args[0] : outPath)
